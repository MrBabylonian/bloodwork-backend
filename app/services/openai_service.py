...
class OpenAIService:
    ...
    This class provides static methods that delegate to the new OOP service.
    It maintains compatibility while encouraging migration to the new structure.
    """

    @staticmethod
    def extract_emogramma_from_image(
        image_paths: List[Path],
        prompt_path: Path
    ) -> str:
        """
        Legacy method for extracting hemogram data from images.

        Args:
            image_paths (List[Path]): List of image file paths
            prompt_path (Path): Path to prompt file

        Returns:
            str: Extracted hemogram data
        """
        service = BloodworkAnalysisService(prompt_path)
        ...
        try:
            result = loop.run_until_complete(
                service.analyze_bloodwork_images(image_paths)
            )
            return result
        finally:
            loop.close()
    ...
    @staticmethod
    async def interpret_bloodwork_via_openai(
        image_path_list: List[Path],
        prompt_path: Path = Path("app/prompts/diagnostic_prompt.txt")
    ) -> str:
        """
        Legacy async method for bloodwork interpretation.

        Args:
            image_path_list (List[Path]): List of image file paths
            prompt_path (Path): Path to diagnostic prompt file

        Returns:
            str: Bloodwork analysis result
        """
        service = BloodworkAnalysisService(prompt_path)
        return await service.analyze_bloodwork_images(image_path_list)
